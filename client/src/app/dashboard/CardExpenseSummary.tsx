        ...
        <div className="row-span-3 bg-white shadow-md rounded-2xl flex flex-col justify-around  xl:-mt-4">
            {isLoading ? (<div className="m-5">Loading...</div>) : (
                <>
                    ...
                    {/* BODY */}
                    <div className=" xl:flex justify-between pr-5 md:-mt-5">
                        {/* CHART */}
                        <div className="relative basis-3/5 z-10">
                            <ResponsiveContainer width="100%" height={200}>
                                <PieChart>
                                    <Pie
                                        ...
                                        cy="50%"
                                    >
                                        {expenseCategories.map((entry, index) => (<Cell key={`cell-${index}`}
                                            fill={colors[index % colors.length]} />))}

                                    </Pie>
                                </PieChart>
                            </ResponsiveContainer>
                            <div className="absolute top-1/2 left-1/2 transform  -translate-x-1/2  -translate-y-1/2 text-center basis-2/5 z-30">
                        ...
                        {/* LABELS */}
                        <ul className="flex flex-col justify-center items-center xl:items-start gap-2 mb-3">
                            {expenseCategories.map((entry, index) => (
                                <li
                                    key={`legend-${index}`}
                                    className="flex items-center text-xs">
                                    <span className="mr-2 w-3 h-3 rounded-full"
                                        style={{ backgroundColor: colors[index % colors.length] }} ></span>{entry.name}
                    ...
                    <div className="xl:-mt-8 md:-mt-5">
                        <hr />
                        {expenseSummary && (
                            <div className="mt-2   flex justify-between items-center px-7 mb-3">
                                <div className="pt-2">
                                ...
                                <span className="flex items-center  ">
                                    <TrendingUp className="mr-2 text-green-800" />
                                    30%
                                </span>
